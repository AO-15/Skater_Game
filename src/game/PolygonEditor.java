...
/**
 * Very basic polygon editor. Use the main method to launch the editor. See
 * below for how it works.
 *
 * THIS IS A QUICK DIRTY HACK. IT IS NOT PROPERLY DOCUMENTED AND WILL NOT BE
 * SUPPORTED. GOOD LUCK.
 *
 * Each time a point is added, deleted or moved, a comma-separated list of
 * coordinates is printed to the console; this text can be copied and pasted as
 * a parameter list for the city.cs.engine.PolygonShape(float... coords)
 * constructor BUT you will have problems if the polygon is not convex (remember
 * though that a Body can have multiple shapes, so you can get pretty much any
 * shaped body by using overlapping convex polygons).
 *
 ...
 * background (allowing you to trace a polygon approximation of the image
 * outline).
 *
 * To add the next point to the polygon, left click where you want it. To delete
 * an existing point, right click on it. To move a point, drag it. It is not
 * currently possible to insert a new point between two existing points; sorry.
 *
 * If the editor is started from the command line, the output from a previous
 * edit can be fed back in for re-editing (after the image file name, if any).
 * The image file name can also be optionally followed by a scale specifier.
 *
 * Example 1:
 *
 * java PolygonEditor yellow-bird.gif -height 2.25
 *
 * lets you draw around the yellow-bird.gif image, generating polygon vertex
 * coordinates on the assumption that the image will be scaled to a height of
 * 2.25 world units (metres) when used in the game (this can be useful if your
 * game involves zooming the display, otherwise the zoomed image will look
 * blocky). If you don't specify the height, it defaults to 1.
 *
 * Example 2:
 *
 * java PolygonEditor yellow-bird.gif -height 2.25 0.149f,0.975f, 0.775f,0.193f,
 * 0.772f,-0.099f, 0.401f,-0.928f, -0.36f,-0.922f, -0.719f,-0.025f,
 * -0.725f,0.163f, -0.14f,0.972f
 *
 * lets you edit the polygon drawn in a previous session around the image
 * yellow-bird.gif).
 */
public class PolygonEditor extends JPanel {
    ...
    /**
     * Initialise a new editor with background image specified by given file
     * name. If file name is null, no image will be loaded.
     *
    ...
    private PolygonEditor(String f, float boxHeight) {
        ...
        if (icon == null) {
            ...
            bitmapWidth = w;
            bitmapHeight = h;
            image = image.getScaledInstance(w * pixelScale, -1, Image.SCALE_DEFAULT);
            icon.setImage(image);
        }
        ...
        setPreferredSize(new java.awt.Dimension(canvasWidth, canvasHeight));
    ...
    /**
     * The current list of polygon vertex coordinates as a comma-separated
     * string.
     * @return 
     */
    ...
    private Point2D.Float toGridPoint(int x, int y) {
        return new Point2D.Float((x - centreX) * scale, (centreY - y) * scale);
    }

    private Point2D.Float screenPosition(Point2D.Float p) {
        return new Point2D.Float(centreX + p.x / scale, centreY - p.y / scale);
    }
    ...
    /**
     * Open an editor. If first command-line argument is not a number, assume it
     * is an image file name. The optional image file name can be optionally
     * followed by a scale specifier of the form -scale (floating-point number)
     * Any remaining arguments are assumed to be a comma-separated list of
     * polygon vertex coordinates, in the same format as the parameter list to
     * city.soi.platform.PolygonShape(float...).
     *
     * @param args command line arguments.
     */
    public static void main(String[] args) {
        String fileName = "data/key.png";
        int firstCoordIndex = 0;
        float boxHeight = 1.75f;
        if (args.length > 0) {
            try {
